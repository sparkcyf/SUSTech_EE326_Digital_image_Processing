...
# define hist
hist = np.concatenate((np.linspace(0, 20, 8 - 0),
                       np.linspace(20, 0.75, 16 - 8),
...
def hist_match(hist_in, hist_desired):
    ...
    def hist_match_index(i):
        index = min(range(hist_len), key=lambda j: abs(hist_desired[j] - hist_in[i]))
        hist_out[i] = index
...
def hist_match_11812418(input_image, spec_hist):
    ...
    img_after_hist_match = hist_lut[input_img_after_hist_equ.astype(int) - 1]

    img_new_arr = np.reshape(img_after_hist_match, img_arr.shape)
...
if __name__ == '__main__':
    ...
    plot_hist(hist, "output/img/hist_matching/hist.png", "Specified Histogram")